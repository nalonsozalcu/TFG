		...
		<div class="col-lg-4">
			...
			<div class="card-body text-center">
				...
				<div class="d-flex justify-content-end mt-2 mb-2">
					<?php if(Usuario::is_favorito($_SESSION["idUsuario"], "restaurante", $restaurante->id())){ ?>
						<a href="../api/favoritos.php?from=ind&action=delete&id=<?php echo (Usuario::is_favorito($_SESSION["idUsuario"], "restaurante", $restaurante->id())) ?>&tipo=restaurante&id_actividad=<?php echo($restaurante->id()) ?>" class="btn ms-1"><i class="bi bi-suit-heart-fill" style="color: red;"></i></a>
					<?php }else{?>
						<a href="../api/favoritos.php?from=ind&action=new&tipo=restaurante&id_actividad=<?php echo($restaurante->id()) ?>" class="btn ms-1"><i class="bi bi-suit-heart" style="color: red;"></i></a>
					<?php }?>
					<?php if($contactos){?>
							...
							<?php foreach($contactos as $user){?>
								<li class="list-group-item">
									<div class="row justify-content-between">
										...
										<?php if(!Usuario::recomendacion_sended($user["id"], $_SESSION["idUsuario"], "restaurante", $restaurante->id())){?>
											<a href="../api/recomendaciones.php?action=send&tipo=restaurante&id_actividad=<?php echo($restaurante->id()) ?>&id_contacto=<?php echo($user["id"]) ?>" class="btn"><i class="bi bi-send-fill text-primary"></i></a>
										<?php } else{?>
			...
			<div class="card-body p-0">
				...
				<li class="list-group-item d-flex justify-content-start align-items-center p-3">
					<?php if(isset($_SESSION["idUsuario"])){ if($restaurante->is_valoracion($restaurante->id(), $_SESSION["idUsuario"])){?>
