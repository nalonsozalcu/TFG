    ...
    if($type === "Museos"){
        for ($row = 2; $row <= $highestRow; $row++){
            if($sheet->getCell("A".$row)->getValue() != "")
                ...
                $sheet->getCell("L".$row)->getValue(), NULL);
        }
        ...
        for ($row = 2; $row <= $highestRow; $row++){
            if($sheet->getCell("A".$row)->getValue() != "")
                ...
                $sheet->getCell("C".$row)->getValue(),  $sheet->getCell("D".$row)->getValue(),  $sheet->getCell("E".$row)->getValue(), 
                $sheet->getCell("F".$row)->getValue(), $sheet->getCell("G".$row)->getValue(),  $sheet->getCell("H".$row)->getValue(),  
                $sheet->getCell("I".$row)->getValue(),  $sheet->getCell("J".$row)->getValue(),  NULL, NULL);
        }
        ...
        for ($row = 2; $row <= $highestRow; $row++){
            if($sheet->getCell("A".$row)->getValue() != "")
                ...
                $sheet->getCell("R".$row)->getValue(), $sheet->getCell("S".$row)->getValue(),NULL, NULL);
        }
    }
